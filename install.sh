...
# sniproxy
echo "${green}Install SNI Proxy...${plain}"
...
lsof -i:80 || (echo "${red}Install SNI Proxy exception...${plain}" && exit 1)
lsof -i:443  || (echo "${red}Install SNI Proxy exception...${plain}" && exit 1)

# nginx
echo "${green}Install Nginx...${plain}"
...
lsof -i:53  || (echo "${red}Install Nginx exception...${plain}" && exit 1)

# systemd
systemctl restart sniproxy dnsmasq nginx
systemctl enable sniproxy dnsmasq nginx

echo "${green}Dnsmasq + SNI Proxy installed！

Change your DNS to ${IP} and you can watch Netflix.${plain}"